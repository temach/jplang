...
{
	if (region.bb.aD === region.bu.aD)
	{
		return 'on line ' + region.bb.aD;
	}
	return 'on lines ' + region.bb.aD + ' through ' + region.bu.aD;
}
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.co, request.be, true);
		} catch (e) {
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {bx: fragment, bA: host, bK: path, bM: port_, bP: protocol, bQ: query};
	});
...
var $author$project$Main$defaultModel = {l: $author$project$Main$defaultCurrentWork, o: 0, M: _List_Nil, aF: 0, j: $elm$core$Dict$empty, x: _List_Nil};
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{b9: false, cc: r.cc, a1: r.a1, by: r.by, co: r.co, cD: r.cD, b0: r.b0, be: r.be}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{cc: $elm$http$Http$emptyBody, a1: r.a1, by: _List_Nil, co: 'GET', cD: $elm$core$Maybe$Nothing, b0: $elm$core$Maybe$Nothing, be: r.be});
};
...
var $author$project$Main$getWorkElements = $elm$http$Http$get(
	{
		a1: A2($elm$http$Http$expectJson, $author$project$Main$WorkElementsReady, $author$project$Main$workElementsDecoder),
		be: A2(
...
var $author$project$Main$KeyCandidate = F3(
	function (word, metadata, freq) {
		return {M: freq, bG: metadata, cI: word};
	});
...
var $author$project$Main$getKeywordCheck = F2(
	function (kanji, keyword) {
		return $elm$http$Http$get(
			{
				a1: A2($elm$http$Http$expectJson, $author$project$Main$KeywordCheckReady, $author$project$Main$keyCandidateDecoder),
				be: A2(
					$elm$url$Url$Builder$relative,
					_List_fromArray(
	...
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'kanji',
				$elm$json$Json$Encode$string(elem.w)),
				_Utils_Tuple2(
				'keyword',
				$elm$json$Json$Encode$string(elem.m)),
				_Utils_Tuple2(
				'notes',
				$elm$json$Json$Encode$string(elem.F))
			]));
};
...
var $elm$http$Http$post = function (r) {
	return $elm$http$Http$request(
		{cc: r.cc, a1: r.a1, by: _List_Nil, co: 'POST', cD: $elm$core$Maybe$Nothing, b0: $elm$core$Maybe$Nothing, be: r.be});
};
	...
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'kanji',
				$elm$json$Json$Encode$string(element.w)),
				_Utils_Tuple2(
				'keyword',
				$elm$json$Json$Encode$string(element.m)),
				_Utils_Tuple2(
				'notes',
				$elm$json$Json$Encode$string(element.F))
			]));
};
var $author$project$Main$submitElement = function (element) {
	return $elm$http$Http$post(
		{
			cc: $elm$http$Http$jsonBody(
				$author$project$Main$submitElementEncoder(element)),
			a1: $elm$http$Http$expectString($author$project$Main$ElementSubmitReady),
			be: A2(
				$elm$url$Url$Builder$relative,
				_List_fromArray(
					['api', 'submit']),
			...
			switch (msg.$) {
				case 5:
					...
					if (!result.$) {
						...
						var newModel = _Utils_update(
							model,
							{x: elements});
						var $temp$msg = $author$project$Main$SelectWorkElement(newModel.o),
							$temp$model = newModel;
						msg = $temp$msg;
						model = $temp$model;
						continue update;
						...
						var newUserMessages = A3($elm$core$Dict$insert, 'WorkElementsReady', message, model.j);
						var newModel = _Utils_update(
							model,
							{j: newUserMessages});
				...
				case 0:
					...
					var cmd = $elm$core$Platform$Cmd$batch(
						_List_fromArray(
							[
								A2($author$project$Main$getKeywordCheck, newModel.l.w, newModel.l.m),
								$author$project$Main$sendMessage(
								$author$project$Main$portEncoder(newModel.l))
							]));
				...
				case 1:
					...
					if (!_v2.$) {
						var value = _v2.a;
						var updatedElement = A3($author$project$Main$WorkElement, value.w, value.m, value.F);
						var updatedWorkElements = A3($elm_community$list_extra$List$Extra$setAt, model.o, updatedElement, model.x);
						var index = model.o + 1;
						...
						var newModel = _Utils_update(
							model,
							{l: currentElement, o: index, aF: model.o, j: $elm$core$Dict$empty, x: updatedWorkElements});
						var cmd = $elm$core$Platform$Cmd$batch(
							_List_fromArray(
								[
									...
									A2($author$project$Main$getKeywordCheck, currentElement.w, currentElement.m)
								]));
						return _Utils_Tuple2(newModel, cmd);
					} else {
				...
				case 6:
					...
					if (!result.$) {
						...
						if ($elm$core$String$length(body) > 0) {
							...
							var newUserMessages = A3($elm$core$Dict$insert, 'ElementSubmitReady', message, model.j);
							var newModel = _Utils_update(
								model,
								{o: model.aF, j: newUserMessages});
				...
				case 4:
					if ($elm$core$String$length(model.l.m) > 0) {
						var updatedWorkElements = A3($elm_community$list_extra$List$Extra$setAt, model.o, model.l, model.x);
						var index = model.o + 1;
						var currentElement = A2(
							$elm$core$Maybe$withDefault,
							$author$project$Main$brokenCurrentWork,
						...
						var cmd = $elm$core$Platform$Cmd$batch(
							_List_fromArray(
								[
									$author$project$Main$submitElement(model.l),
									$author$project$Main$sendMessage(
									$author$project$Main$portEncoder(currentElement)),
									A2($author$project$Main$getKeywordCheck, currentElement.w, currentElement.m)
								]));
						...
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									j: A3($elm$core$Dict$insert, 'ElementSubmitClick', '{{ submit_onclick_error }}', model.j)
								}),
				...
				case 2:
					...
					var cmd = $elm$core$Platform$Cmd$batch(
						_List_fromArray(
							[
								A2($author$project$Main$getKeywordCheck, newCurrentWork.w, newCurrentWork.m),
								$author$project$Main$sendMessage(
								$author$project$Main$keywordEncoder(newCurrentWork.m))
							]));
					return ($elm$core$String$length(word) >= 2) ? _Utils_Tuple2(newModel, cmd) : _Utils_Tuple2(
						_Utils_update(
							newModel,
							{M: _List_Nil, j: $elm$core$Dict$empty}),
						$elm$core$Platform$Cmd$none);
				case 3:
					var word = msg.a;
					var current = model.l;
					...
					if (!result.$) {
						...
						return _Utils_Tuple2(
							_Utils_update(
								...
								{
									M: elem.M,
									j: A3($elm$core$Dict$insert, 'KeywordCheckReady', elem.bG, model.j)
								}),
							$elm$core$Platform$Cmd$none);
					} else {
						return _Utils_Tuple2(
							_Utils_update(
								...
								{
									M: _List_Nil,
									j: A3($elm$core$Dict$insert, 'KeywordCheckReady', '{{ error_keyword_check }}', model.j)
								}),
							$elm$core$Platform$Cmd$none);
					}
			}
...
var $author$project$Main$ElementSubmitClick = {$: 4};
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text(currentWork.w)
						])),
					A2(
					$elm$html$Html$span,
					_List_fromArray(
						...
						[
							...
							_List_fromArray(
								[
									...
									$elm$html$Html$Attributes$value(currentWork.m),
						...
						[
							...
							_List_fromArray(
								[
									...
									$elm$html$Html$Attributes$value(currentWork.F),
				...
				[
					...
					_List_fromArray(
						[
							...
							A2($elm$html$Html$Attributes$style, 'margin', '0 0.5rem'),
							($elm$core$String$length(elem.m) > 0) ? A2($elm$html$Html$Attributes$style, 'background-color', 'rgb(200, 210, 200)') : A2($elm$html$Html$Attributes$style, 'background-color', '')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text(elem.m)
						])),
					A2(
					$elm$html$Html$span,
					_List_fromArray(
						[
							...
							($elm$core$String$length(elem.F) > 0) ? A2($elm$html$Html$Attributes$style, 'background-color', 'rgb(200, 200, 210)') : A2($elm$html$Html$Attributes$style, 'background-color', '')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text(elem.F)
						]))
				]));
	});
...
var $author$project$Main$renderWorkElements = function (model) {
	...
	return A2(
		...
		A2($elm$core$List$indexedMap, partial, model.x));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A3($elm$html$Html$Lazy$lazy2, $author$project$Main$renderSubmitBar, model.l, model.M),
