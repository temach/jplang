...
function touchstart_handler(ev) {
    if (ev.touches.length == 1) {
        window.top.halfPinchZoom = true;
    } else if (ev.touches.length == 2) {
        // it touching twice on this doc, then no need to tell other iframes
        // of halfpinchzoom, becuase it proper pinch zoom already
        window.top.halfPinchZoom = false;
    }
}

function touchend_handler(ev) {
    // if (ev.touches.length < 2) {
    //     console.log('finger lifted, enabling iframe');
    //     // the pinch zoom has ended, revert pointer-events
