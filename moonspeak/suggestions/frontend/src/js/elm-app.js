...
var $author$project$Main$defaultModel = {kanji: 'X', keyword: '{{ loading }}', suggestions: _List_Nil, userMessages: $elm$core$Dict$empty};
...
var $author$project$Main$SortSuggestionsByFreq = {$: 'SortSuggestionsByFreq'};
...
var $author$project$Main$SuggestionsReady = function (a) {
	return {$: 'SuggestionsReady', a: a};
};
...
var $author$project$Main$suggestionsDecoder = $elm$json$Json$Decode$list($author$project$Main$keyCandidateDecoder);
var $author$project$Main$getSuggestions = function (kanji) {
	return $elm$http$Http$get(
		{
			expect: A2($elm$http$Http$expectJson, $author$project$Main$SuggestionsReady, $author$project$Main$suggestionsDecoder),
			url: A2(
				$elm$url$Url$Builder$relative,
				_List_fromArray(
					['api', 'suggestions/' + kanji]),
				_List_Nil)
		});
};
	...
	function (msg, model) {
		...
		while (true) {
			switch (msg.$) {
				case 'SelectSuggestion':
					...
					var newSuggestion = A2(
						...
						A3(
							...
							_List_fromArray(
								[0])),
						A2($elm_community$list_extra$List$Extra$getAt, index, model.suggestions));
				...
				case 'SortSuggestionsByFreq':
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								suggestions: $elm$core$List$reverse(
									A2($elm$core$List$sortWith, $author$project$Main$compareKeyCandidates, model.suggestions))
							}),
						$elm$core$Platform$Cmd$none);
				case 'SortSuggestionsByOrigin':
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
				...
				case 'SuggestionsReady':
					var result = msg.a;
					if (result.$ === 'Ok') {
						var suggestions = result.a;
						if ($elm$core$List$length(suggestions) > 0) {
							var messages = A2($elm$core$Dict$remove, 'SuggestionsReady', model.userMessages);
							var newModel = _Utils_update(
								model,
								{suggestions: suggestions, userMessages: messages});
							var $temp$msg = $author$project$Main$SortSuggestionsByFreq,
								$temp$model = newModel;
							msg = $temp$msg;
							model = $temp$model;
							continue update;
						} else {
							var errmsg = A3($elm$core$String$replace, '${kanji}', model.kanji, '{{ suggestions_are_empty }}');
							var messages = A3($elm$core$Dict$insert, 'SuggestionsReady', errmsg, model.userMessages);
							return _Utils_Tuple2(
								_Utils_update(
									model,
						...
						var errmsg = A3($elm$core$String$replace, '${kanji}', model.kanji, '{{ error_getting_suggestions }}');
						var messages = A3($elm$core$Dict$insert, 'SuggestionsReady', errmsg, model.userMessages);
						return _Utils_Tuple2(
							_Utils_update(
								model,
						...
						if (_Utils_eq(value.kanji, model.kanji)) {
							...
							return _Utils_Tuple2(
								...
								$elm$core$Platform$Cmd$batch(
									_List_fromArray(
										[
											$author$project$Main$getSuggestions(newModel.kanji)
										])));
						}
					} else {
						return _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
...
var $author$project$Main$SortSuggestionsByOrigin = {$: 'SortSuggestionsByOrigin'};
var $author$project$Main$SelectSuggestion = function (a) {
	return {$: 'SelectSuggestion', a: a};
};
var $author$project$Main$renderSingleSuggestion = F3(
	function (model, index, suggest) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							A2($elm$html$Html$Attributes$style, 'flex', '0 0 6rem')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text(suggest.metadata + ': ')
						])),
					A2(
					$elm$html$Html$span,
					_List_fromArray(
						[
							A2($elm$html$Html$Attributes$style, 'flex', '10 0 6rem')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text(suggest.word)
						])),
					A2(
					$elm$html$Html$span,
					_List_fromArray(
						[
							A2($elm$html$Html$Attributes$style, 'flex', '1 0 3rem')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text(
							$elm$core$String$fromInt(
								A2(
									$elm$core$Maybe$withDefault,
									0,
									A2($elm_community$list_extra$List$Extra$getAt, 0, suggest.freq))))
						])),
					A2(
					$elm$html$Html$span,
					_List_fromArray(
						...
						[
							...
							$elm$core$String$fromInt(
								A2(
									$elm$core$Maybe$withDefault,
									0,
									A2($elm_community$list_extra$List$Extra$getAt, 1, suggest.freq))))
						]))
				]));
	});
...
var $author$project$Main$renderSuggestions = function (model) {
	var partial = $author$project$Main$renderSingleSuggestion(model);
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				...
				$elm$html$Html$Events$on,
				'click',
				A2($elm$json$Json$Decode$map, $author$project$Main$SelectSuggestion, $elm_community$html_extra$Html$Events$Extra$targetValueIntParse))
			]),
		A2($elm$core$List$indexedMap, partial, model.suggestions));
};
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								A2($elm$html$Html$Attributes$style, 'flex', '10 0 calc(1.5rem + 6rem + 6rem)'),
								$elm$html$Html$Events$onClick($author$project$Main$SortSuggestionsByOrigin)
							]),
						_List_fromArray(
							[
								$elm$html$Html$text('{{ title }}')
						...
						_List_fromArray(
							[
								A2($elm$html$Html$Attributes$style, 'flex', '1 0 3rem'),
								$elm$html$Html$Events$onClick($author$project$Main$SortSuggestionsByFreq)
							]),
						_List_fromArray(
							[
				...
				$author$project$Main$renderSuggestions(model)
			]));
};
...
_Platform_export({'Main':{'init':$author$project$Main$main(
	$elm$json$Json$Decode$succeed(_Utils_Tuple0))({"versions":{"elm":"0.19.1"},"types":{"message":"Main.Msg","aliases":{"Main.KeyCandidate":{"args":[],"type":"{ word : String.String, metadata : String.String, freq : List.List Basics.Int }"},"Json.Decode.Value":{"args":[],"type":"Json.Encode.Value"}},"unions":{"Main.Msg":{"args":[],"tags":{"SelectSuggestion":["Basics.Int"],"SortSuggestionsByFreq":[],"SortSuggestionsByOrigin":[],"SuggestionsReady":["Result.Result Http.Error (List.List Main.KeyCandidate)"],"Recv":["Json.Decode.Value"]}},"Http.Error":{"args":[],"tags":{"BadUrl":["String.String"],"Timeout":[],"NetworkError":[],"BadStatus":["Basics.Int"],"BadBody":["String.String"]}},"Basics.Int":{"args":[],"tags":{"Int":[]}},"List.List":{"args":["a"],"tags":{}},"Result.Result":{"args":["error","value"],"tags":{"Ok":["value"],"Err":["error"]}},"String.String":{"args":[],"tags":{"String":[]}},"Json.Encode.Value":{"args":[],"tags":{"Value":[]}}}}})}});}(window));
