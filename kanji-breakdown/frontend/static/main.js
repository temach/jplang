...
var $author$project$Main$PartsReady = function (a) {
	return {$: 'PartsReady', a: a};
};
...
var $author$project$Main$getParts = $elm$http$Http$get(
	{
		expect: A2($elm$http$Http$expectJson, $author$project$Main$PartsReady, $author$project$Main$workElementsDecoder),
		url: A2(
...
var $author$project$Main$init = function (_v0) {
	...
	var commands = $elm$core$Platform$Cmd$batch(
		_List_fromArray(
			[$author$project$Main$getWorkElements, $author$project$Main$getParts]));
...
var $author$project$Main$updateWorkElement = F3(
	function (index, newElement, list) {
		var updator = F2(
			function (i, elem) {
				return _Utils_eq(i, index) ? newElement : elem;
			});
		return A2($elm$core$List$indexedMap, updator, list);
	});
		...
		while (true) {
			switch (msg.$) {
				case 'KeywordSubmitClick':
					return ($elm$core$String$length(model.keyword) > 0) ? _Utils_Tuple2(
						...
						_Utils_update(
							model,
							{
								userMessage: A3($elm$core$Dict$insert, 'KeywordSubmitClick', 'Error: keyword length must be non-zero', model.userMessage)
							}),
						$elm$core$Platform$Cmd$none);
				case 'KeywordSubmitReady':
					var result = msg.a;
					if (result.$ === 'Ok') {
						var body = result.a;
						...
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									userMessage: A3($elm$core$Dict$insert, 'KeywordSubmitReady', 'Error submitting keyword. Details unknown.', model.userMessage)
								}),
							$elm$core$Platform$Cmd$none);
					}
				...
				case 'SelectWorkElement':
					var index = msg.a;
					var keywordAbsentCommands = $elm$core$Platform$Cmd$batch(_List_Nil);
					var fixWorkModel = A2($author$project$Main$chooseWorkElement, index, model);
					var newModel = _Utils_update(
						fixWorkModel,
						{userMessage: $elm$core$Dict$empty});
				...
				case 'WorkElementsReady':
					...
					if (result.$ === 'Ok') {
						...
						var newModel = _Utils_update(
							model,
							{
								userMessage: A2($elm$core$Dict$remove, 'WorkElementsReady', model.userMessage),
						...
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									userMessage: A3($elm$core$Dict$insert, 'WorkElementsReady', 'Error getting workElements', model.userMessage)
								}),
							$elm$core$Platform$Cmd$none);
					}
				case 'PartsReady':
					var result = msg.a;
					if (result.$ === 'Ok') {
						var newParts = result.a;
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									parts: newParts,
									userMessage: A2($elm$core$Dict$remove, 'PartsReady', model.userMessage)
								}),
							$elm$core$Platform$Cmd$none);
					} else {
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									userMessage: A3($elm$core$Dict$insert, 'PartsReady', 'Error getting parts', model.userMessage)
								}),
							$elm$core$Platform$Cmd$none);
					}
...
var $author$project$Main$cleanupSvg = F2(
	function (customiser, svgNode) {
		switch (svgNode.$) {
			case 'SvgElement':
				var element = svgNode.a;
				var customisedAttributes = customiser(element);
				var cleanupSvgWithCustomiser = $author$project$Main$cleanupSvg(customiser);
				return A3(
					$elm$svg$Svg$node,
					element.name,
					customisedAttributes,
					A2($elm$core$List$map, cleanupSvgWithCustomiser, element.children));
			case 'SvgText':
...
var $author$project$Main$stringToSvgHtml = F2(
	function (customiser, svgString) {
		var _v0 = $Garados007$elm_svg_parser$SvgParser$parseToNode(svgString);
		if (_v0.$ === 'Ok') {
			var svgNode = _v0.a;
			return A2($author$project$Main$cleanupSvg, customiser, svgNode);
		} else {
			var e = _v0.a;
...
var $author$project$Main$getAttributeValue = F2(
	function (attributeName, element) {
		var searchAttribute = function (_v2) {
			var name = _v2.a;
			var value = _v2.b;
			return _Utils_eq(name, attributeName);
		};
		var _v0 = A2($elm_community$list_extra$List$Extra$find, searchAttribute, element.attributes);
		if (_v0.$ === 'Nothing') {
			var _v1 = A2($elm$core$Debug$log, 'PANIC', element.attributes);
			return '';
		} else {
			var idAttr = _v0.a;
			return idAttr.b;
...
var $author$project$Main$svgCustomiseHighlight = function (element) {
	var finalAttributes = function () {
		if (!$elm$core$List$length(element.children)) {
			var newStyle = _List_fromArray(
				[
					$elm$svg$Svg$Attributes$class('svgCentralHighlight')
				]);
			var isStyle = function (_v0) {
				var name = _v0.a;
				var value = _v0.b;
				return name === 'style';
			};
			var noStyle = A2($elm_community$list_extra$List$Extra$filterNot, isStyle, element.attributes);
			var idAttr = A2($author$project$Main$getAttributeValue, 'id', element);
			var clickAttributes = _List_fromArray(
				[
					$elm$html$Html$Events$onClick(
					$author$project$Main$SvgClick(idAttr))
				]);
			var base = A2($elm$core$List$map, $Garados007$elm_svg_parser$SvgParser$toAttribute, noStyle);
			return _Utils_ap(
				base,
				_Utils_ap(newStyle, clickAttributes));
		} else {
			var base = A2($elm$core$List$map, $Garados007$elm_svg_parser$SvgParser$toAttribute, element.attributes);
			return base;
		}
...
var $author$project$Main$renderSubmitBar = function (model) {
	var join = F2(
		function (s1, s2) {
			return s1 + (', ' + s2);
		});
			...
			[
				...
				_List_fromArray(
					[
						...
						A2($elm$html$Html$Attributes$style, 'background-color', 'rgb(250, 210, 210)')
					]),
				_List_fromArray(
					[
				...
				[
					...
					_List_fromArray(
						[
							...
							($elm$core$String$length(elem.note) > 0) ? A2($elm$html$Html$Attributes$style, 'background-color', 'rgb(200, 200, 210)') : A2($elm$html$Html$Attributes$style, 'background-color', '')
						]),
					_List_fromArray(
						[
							$elm$html$Html$text(elem.note)
						]))
				]));
	});
			...
			[
				...
				_List_fromArray(
					[
						...
						$author$project$Main$renderSubmitBar(model)
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
				...
				_List_fromArray(
					[
						A2($elm$html$Html$Attributes$style, 'background-color', 'rgb(250, 210, 250)'),
						A2($elm$html$Html$Attributes$style, 'grid-column', '3 / 4'),
						A2($elm$html$Html$Attributes$style, 'grid-row', '2 / 4'),
						A2($elm$html$Html$Attributes$style, 'overflow', 'auto')
					]),
				_List_fromArray(
